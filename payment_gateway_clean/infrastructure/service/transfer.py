...
class TransferService(ITransferService):
    ...
    async def get_transfers(self) -> Union[ApiResponse[List[TransferResponse]], ErrorResponse]:
        try:
            ...
            transfer_responses = [TransferResponse.from_dtos(transfer) for transfer in transfers]
    ...
    async def get_transfer(self, id: int) -> Union[ApiResponse[Optional[TransferResponse]], ErrorResponse]:
        try:
            ...
            return ErrorResponse(
                status="error",
                message=f"Failed to retrieve transfer with id {id}"
            )
    ...
    async def get_transfer_user(
        ...
        try:
            ...
            return ErrorResponse(
                status="error",
                message=f"Failed to retrieve transfer for user {id}"
            )
    ...
    async def create_transfer(
        ...
        try:
            ...
            if sender is None:
                ...
                raise NotFoundError(
                    f"User with id {input.transfer_from} not found"
                )
            ...
            if receiver is None:
                ...
                raise NotFoundError(
                    f"User with id {input.transfer_to} not found"
                )

            # Create the transfer record
            transfer = await self.transfer_repository.create(input)
            
            # Update the sender's saldo
            sender_saldo = await self.saldo_repository.find_by_user_id(input.transfer_from)
            ...
            # Rollback transfer if saldo update fails
            try:
                ...
                return ErrorResponse(
                    status="error",
                    message=f"Failed to update saldo balance for sender"
                )

            # Update the receiver's saldo
            receiver_saldo = await self.saldo_repository.find_by_user_id(input.transfer_to)
            ...
            try:
                ...
                return ErrorResponse(
                    status="error",
                    message=f"Failed to update saldo balance for receiver"
                )
            ...
            return ErrorResponse(
                status="error",
                message=f"Failed to create transfer:"
            )
    ...
    async def update_transfer(
        ...
        try:
            ...
            sender_saldo = await self.saldo_repository.find_by_user_id(transfer.transfer_from)
            if not sender_saldo:
            ...
            try:
                ...
                return ErrorResponse(
                    status="error",
                    message=f"Failed to update sender's saldo: "
                )

            # Update receiver's saldo
            receiver_saldo = await self.saldo_repository.find_by_user_id(transfer.transfer_to)
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"Failed to rollback sender's saldo update: {rollback_err}")

                return ErrorResponse(
                    f"Failed to rollback sender's saldo update"
                )
            ...
            return ErrorResponse(
                status="error",
                message=f"Failed to update transfer"
            )

    async def delete_transfer(
        self, id: int
    ) -> Union[ApiResponse[None], ErrorResponse]:
        # Retrieve the user
        user = await self.user_repository.find_by_id(id)
        if not user:
            logger.error(f"User with id {id} not found")
        ...
        existing_transfer = await self.transfer_repository.find_by_user(user.user_id)
        if existing_transfer:
            try:
                ...
                logger.error(f"Failed to delete transfer for user id {id}: {db_err}")

                raise NotFoundError(
                    f"Failed to delete transfer for user id {id}"
                )
        else:
            logger.error(f"Transfer with user id {id} not found")

            raise NotFoundError(
                f"Transfer with user id {id} not found"
            )
